...
class Trainer(TrainerAbstract, TrainerLoss, TrainerIteration, TrainerDataset, TrainerModel):
    ...
    def train_loop(self):
        ...
        for data in iterator:
            ...
            self.cum_iteration = self.cum_iteration + 1
            self.data = EasyDict(data)
            self.data.points = self.data.points.to(self.opt.device)

            if self.opt.SVR and self.opt.mode == 'viewer':
                self.data.cam_rotmat = self.data.cam_rotmat.to(self.opt.device)
                self.data.points = torch.bmm(self.data.points, torch.transpose(self.data.cam_rotmat, 1, 2))
    ...
    def test_epoch(self):
        ...
        self.network.eval()
        self.test_loop()
        self.log.end_epoch(self.opt.logger, self.opt.tbwriter, self.epoch)

        try:
            self.log.update_curves(self.opt.dir_name)
        except:
            self.opt.logger.info("could not update curves")
        self.opt.logger.info(f"Sampled {self.num_val_points} regular points for evaluation")
    ...
    def demo(self, demo_path, input_path_points=None):
        ...
        self.opt.logger.info(f"Atlasnet generated mesh at {path}!")
        mesh_processor.save(mesh, path, self.colormap)
        return path
