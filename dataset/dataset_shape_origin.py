...
if __name__ == '__main__':
    ...
    opt = {"normalization": "UnitBall", "class_choice": ["plane"], "SVR": True, "sample": True, "npoints": 2500,
           ...
           "autoaug_type": 'RGB', "number_points":2500, "mag_idx":0, "prob":0.0, "n_op":0}
    opt = EasyDict(opt)
    my_utils.plant_seeds(opt.manual_seed)
    if_train = True
    dataset = ShapeNet(opt, train=if_train)
    ...
    for cat in train_test_dic:
        if if_train:
            fo = open(f"data/origin_filelists/{cat}_train.lst", "w")
        else:
            fo = open(f"data/origin_filelists/{cat}_test.lst", "w")
