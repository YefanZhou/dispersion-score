...
parser.add_argument('--y_rot_range', type=int, default=0,
                    help='range of y rot to random sample')
parser.add_argument('--views', type=int, default=1, help='num of views to generate')
...
#	os.makedirs(obj_normal_hard_dir, exist_ok=True)
	if os.path.exists(os.path.join(obj_image_easy_dir, "rendering_metadata.txt")):
		print("Exist!!!, skip %s %s" % (cat_id, obj_id))
	else:
		print("Start %s %s" % (cat_id, obj_id))
		if FLAGS.debug:
			os.system(FLAGS.blender_location + ' --background --python render_blender_anglecontrol.py -- --views %d --y_rot_range %d --obj_image_easy_dir %s %s ' % (FLAGS.views, FLAGS.y_rot_range, obj_image_easy_dir, objpath))

		else:
			os.system(FLAGS.blender_location + ' --background --python render_blender_anglecontrol.py -- --views %d --y_rot_range %d --obj_image_easy_dir %s %s > /dev/null 2>&1' % (FLAGS.views, FLAGS.y_rot_range, obj_image_easy_dir, objpath))
...
print(f"Elasped Time: {(time.time()-start_time)/60/60} h")
