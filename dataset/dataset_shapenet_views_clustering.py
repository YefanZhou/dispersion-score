...
#############################################################################################
        ...
        if self.pretrained:
            self.opt.logger.info("PreTrained Normalization")
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            return_dict['label'] = torch.tensor(self.cls_labels[index]).long()
        else:
            return_dict['label'] = -1
...
if __name__ == '__main__':
    ...
    opt = {"normalization": "UnitBall", "class_choice": ["plane"], "SVR": True, "sample": True, "npoints": 2500,
           ...
           "autoaug_type": 'CIFAR10', "number_points":2500, "mag_idx":4, "prob":0.5, "n_op":2}
    opt = EasyDict(opt)
    ...
    res_path = f'data/color_aug_verify/train{if_train}_{opt.img_aug_type}_{opt.autoaug_type}_op{opt.n_op}.png'
