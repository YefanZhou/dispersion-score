...
for seed_idx in range(num_seed):
    ...
    if opt.split == "train":
        ...
        dataset = ShapeNet(opt, train=False, num_image_per_object=nviews_dic[opt.split]) 
    else:
...
np.savez_compressed(os.path.join(res_path, f"{opt.mode}_{opt.split}_{opt.type}_{opt.img_aug_type}_{opt.autoaug_type}_mag{opt.mag_idx}_magnitude{opt.magnitude}_n_op{opt.n_op}_prob{opt.prob}_trainnv{opt.nviews_train:02}_testnv{opt.nviews_test:02}_{sample_num}.npz"), **sscore_collect)
